  ...
  async function downloadPDF() {
    if (!result?.pdf_path) {
      ...
      try {
        ...
        if (data.pdf_path) {
          const filename = data.pdf_path.split(/[\\/]/).pop(); // Extract filename
          window.location.href = `/api/report/download?filename=${encodeURIComponent(filename)}`;
        }
      } catch (e) {
        setError(String(e.message || e));
      } finally {
      ...
      const filename = result.pdf_path.split(/[\\/]/).pop(); // Extract filename
      window.location.href = `/api/report/download?filename=${encodeURIComponent(filename)}`;
    }
  }
  ...
  const reportContent = result ? (
    <div style={{ maxHeight: "70vh", overflowY: "auto" }}>
      ...
      <div style={{ marginBottom: 40 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--text-muted)" }}>{orgName}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "linear-gradient(135deg, #1f2937, #111827)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 24, marginBottom: 24 }}>
        <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12, fontWeight: 800, textTransform: "uppercase" }}>Your Security Score</div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div className="grid grid-2" style={{ gap: 12, marginBottom: 12 }}>
          <div style={{ background: "#1f2937", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 6, fontWeight: 800 }}>Security Events Detected</div>
            <div style={{ fontSize: 32, fontWeight: 800, color: "#3B82F6" }}>{result.summary.siem?.events || 0}</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 8 }}>Total suspicious activities across your systems</div>
          </div>
          <div style={{ background: "#1f2937", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: 16 }}>
            ...
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 8 }}>Scanned with {result.summary.virustotal?.total_engines || 0} security engines</div>
          </div>
        </div>
      </div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            ...
            {result.details.vulnerabilities.length > 10 && (
              <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 8, padding: 12, textAlign: "center", color: "var(--text-muted)" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {result.details?.defence_checks && result.details.defence_checks.length > 0 && (
            <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
              <div style={{ fontSize: 12, color: "var(--text-muted)", fontWeight: 800 }}>DETAILED CHECKS</div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            ...
            {result.details.siem_events.length > 8 && (
              <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 8, padding: 10, textAlign: "center", color: "var(--text-muted)", fontSize: 11 }}>
      ...
      {/* Report Info */}
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 16, color: "var(--text-muted)", fontSize: 10 }}>
            ...
            {result && (
              <>
                <div style={{ borderTop: "1px solid rgba(67,70,85,0.15)", paddingTop: 12, marginTop: 12 }}>
                  <div style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 800, letterSpacing: "0.14em", marginBottom: 8 }}>DOWNLOAD OPTIONS</div>
                ...
                {result.pdf_path && (
                  <div style={{ background: "#1f2937", border: "1px solid rgba(52,199,89,0.3)", borderRadius: 6, padding: 10, marginTop: 12 }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--text-muted)", wordBreak: "break-all" }}>{result.pdf_path}</div>
                  </div>
                )}
              </>
            )}
